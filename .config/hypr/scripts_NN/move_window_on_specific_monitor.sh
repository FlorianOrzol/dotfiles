...
SWITCH_AFTER_MOVE=$3 # "true" or "false"

if [ -z "$DIRECTION" ] || [ -z "$TARGET_MONITOR_NAME" ] || [ -z "$SWITCH_AFTER_MOVE" ]; then
    echo "Usage: $0 <next|prev> <monitor_name> <true|false>"
    exit 1
fi
...
if [ -n "$NEXT_WORKSPACE_NAME" ]; then
    ...
    hyprctl workspaces -j | grep -q "\"name\":\"$NEXT_WORKSPACE_NAME\""
    if [ $? -ne 0 ]; then
        hyprctl dispatch workspace "$NEXT_WORKSPACE_NAME"
        hyprctl dispatch moveworkspacetomonitor "$NEXT_WORKSPACE_NAME" "$TARGET_MONITOR_NAME"
    fi

    hyprctl dispatch movetoworkspace "$NEXT_WORKSPACE_NAME"
    if [ "$SWITCH_AFTER_MOVE" == "true" ]; then
    ...
    # If no workspaces exist, create the first one and move window to it
    ...
    hyprctl dispatch movetoworkspace "$FIRST_WS_NAME"
    if [ "$SWITCH_AFTER_MOVE" == "true" ]; then
        hyprctl dispatch focusmonitor "$TARGET_MONITOR_NAME"
        hyprctl dispatch workspace "$FIRST_WS_NAME"
    fi
