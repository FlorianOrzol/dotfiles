...
function extension_start {
    ...
    if (( action_count == 0 )); then
        ERROR "No action specified. Please provide --cmd, --alias, --delete, --edit, or --list."
        return 1
    elif (( action_count > 1 )); then
        ERROR "Multiple actions specified. Please provide only one action at a time."
        return 1
    fi

    # 3. --- Action Routing ---------------
    if [[ -n "$ARG_LIST" ]];   then action_list;                                   return $?; fi
    if [[ -n "$ARG_DELETE" ]]; then action_delete;                                 return $?; fi
    if [[ -n "$ARG_EDIT" ]];   then action_edit;                                   return $?; fi
    if [[ -n "$ARG_ALIAS" ]];  then action_alias  "$device_type" "$target_device"; return $?; fi
    if [[ -n "$ARG_CMD" ]];    then action_cmd     "$device_type" "$target_device"; return $?; fi
}
...
function action_edit {
    ...
    if [[ -n "$ARG_EDIT_ALIAS" ]];   then update_data+=("alias"       "$ARG_EDIT_ALIAS");   fi
    if [[ -n "$ARG_EDIT_CMD" ]];     then update_data+=("command"     "$ARG_EDIT_CMD");     fi
    if [[ -n "$ARG_EDIT_DESC" ]];    then update_data+=("description" "$ARG_EDIT_DESC");    fi
    if [[ -n "$ARG_EDIT_DEVICES" ]]; then update_data+=("devices"     "$ARG_EDIT_DEVICES"); fi

    if (( ${#update_data[@]} == 0 )); then
        ERROR "No edit options provided. Use --edit_alias, --edit_cmd, --edit_desc, or --edit_devices."
        return 1
    fi
...
# @parameter        : $2 | id   | The ID of the target device.
# ==============================================================================
function action_cmd {
    local type="$1" id="$2"

    if [[ -n "$ARG_SAVE" ]]; then
        local desc="${ARG_DESC:-}" devices_str=""

        # Join --multi devices array into comma-separated string for storage
...
function action_alias {
    ...
    declare -a result
    lx db --file "cmds.db" --table "commands" --select @result --cols "command" \
        --where "alias='$ARG_ALIAS'" --limit 1

    local cmd_string="${result[0]:-}"

    if [[ -z "$cmd_string" ]]; then
        ERROR "Alias '$ARG_ALIAS' not found in cmds.db."
        return 1
    fi

    lx output --info "Executing alias '$ARG_ALIAS'..."
    execute_ssh "$type" "$id" "$cmd_string"
}
...
# @parameter        : $1 | type | The type of the device.
# @parameter        : $2 | id   | The ID of the device.
...
function execute_ssh {
    ...
    if ! run_on_device "$type" "$id" "$cmd"; then
