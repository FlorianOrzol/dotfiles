function extension_start() {
    ...
    # 1. Resolve arguments: Did the user use '--files' or just positional arguments?
    ...
    for path in "${target_files[@]}"; do
        ...
        if question "Add '$abs_path' to public (and private) repo?" --default-no; then
            ...
            # Force add (-f) ensures it gets tracked even if ignored by .gitignore
            if lx cmd --run "/usr/bin/git --git-dir=$public_git --work-tree=$HOME add -f '$abs_path'"; then
                # Only add to private if public succeeded
