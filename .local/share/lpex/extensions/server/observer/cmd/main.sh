#!/bin/bash
# ==============================================================================
# @meta_module      : server observer cmd
# @meta_file        : main.sh
# @meta_date        : 2026-04-11
#
# @desc_short       : Executes raw bash commands or saved macros on an Observer Pi.
...
#
# @notes            : Uses -t flag for SSH to support interactive sudo prompts in macros.
# ==============================================================================
...
function extension_start() {
    ...
    if (( ARG_LIST )); then
        ...
        results=$(sqlite3 "$PATH_EXTENSION_DATA/commands.db" \
            "SELECT alias, command FROM device_commands WHERE target_type='observer' AND target_id='$node' ORDER BY alias;" \
            2>/dev/null)
    ...
    if (( ARG_DELETE )); then
        ...
        local sql="DELETE FROM device_commands WHERE target_type='observer' AND target_id='$node' AND alias='$sql_alias';"
    ...
    if (( ARG_RUN )) && [[ -z "$raw_cmd" ]]; then
        ...
        raw_cmd=$(sqlite3 "$PATH_EXTENSION_DATA/commands.db" \
            "SELECT command FROM device_commands WHERE target_type='observer' AND target_id='$node' AND alias='$alias_name';" \
            2>/dev/null)
        if [[ -z "$raw_cmd" ]]; then
    ...
    if (( ARG_SAVE )); then
        ...
        if (( ARG_SAVE )); then
            ...
            local sql="REPLACE INTO device_commands (target_type, target_id, alias, command) VALUES ('observer', '$node', '$sql_alias', '$sql_cmd');"
            if lx db --file "commands.db" --exec "$sql" --quiet; then
    ...
    if (( ARG_RUN )); then
        ...
        # [LOGIC] Encode the command to Base64 so any special characters, quotes
        # or newlines survive the SSH argument chain intact. -t allocates a PTY
        # for interactive sudo prompts inside macros.
        local encoded_cmd
        encoded_cmd=$(printf '%s' "$raw_cmd" | base64 -w0)

        lx cmd --run "ssh -t $USER_OBSERVER@$ip \"printf '%s' '$encoded_cmd' | base64 -d | bash\"" \
               --log --log-tags "$tag" \
               --no-error-msg
    else
