...
# ==============================================================================
# --- Main Execution ---
# Module: server host cmd
# Description: Executes raw bash commands or saved macros on a Bare-Metal Host.
# ==============================================================================
...
function extension_start() {
    ...
    if (( ARG_DELETE )); then
        ...
        local sql="DELETE FROM device_commands WHERE target_type='host' AND target_id='$node' AND alias='$alias_name';"
    ...
    if (( ARG_RUN )) && [[ -z "$raw_cmd" ]]; then
        ...
        raw_cmd=$(sqlite3 "$PATH_EXTENSION_DATA/commands.db" "SELECT command FROM device_commands WHERE target_type='host' AND target_id='$node' AND alias='$alias_name';" 2>/dev/null)
        if [[ -z "$raw_cmd" ]]; then
    ...
    if (( ARG_SAVE )); then
        ...
        local existing_cmd
        existing_cmd=$(sqlite3 "$PATH_EXTENSION_DATA/commands.db" "SELECT command FROM device_commands WHERE target_type='host' AND target_id='$node' AND alias='$alias_name';" 2>/dev/null)
        ...
        if (( ARG_SAVE )); then
            ...
            local sql="REPLACE INTO device_commands (target_type, target_id, alias, command) VALUES ('host', '$node', '$alias_name', '$raw_cmd');"
            if lx db --file "commands.db" --exec "$sql" --quiet; then
    ...
    if (( ARG_RUN )); then
        ...
        lx cmd --run "ssh -t $USER_PVE@$ip \"$raw_cmd\"" \
               --log --log-tags "$tag" \
               --no-error-msg
    else
