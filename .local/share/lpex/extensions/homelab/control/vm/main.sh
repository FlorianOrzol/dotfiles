...
# ==============================================================================
# @meta_name        : main.sh
# @desc_short       : Validates and routes power and maintenance actions for VMs.
# ==============================================================================
...
function extension_start {
    [[ -z "$ARG_VM" ]] && { ERROR "No VM specified. Use --vm <id>."; return 1; }  # device required
    ...
    [[ -n "$ARG_MAINTENANCE" ]] && { action_maintenance "$ARG_VM" || return 1; }   # state change before power
    [[ -n "$ARG_ACTIVATE" ]]    && { action_activate    "$ARG_VM" || return 1; }
    [[ -n "$ARG_START" ]]   && action_power "$ARG_VM" "start"
    [[ -n "$ARG_STOP" ]]    && action_power "$ARG_VM" "stop"
    [[ -n "$ARG_RESTART" ]] && action_power "$ARG_VM" "restart"
}
